...
var initConfig = {
    ...
    appId: config.appid,
    mchId: "1452622302",
    notifyUrl: "http://teach.wanviv.com/weixin/pay",
...
exports = module.exports = function (app) {
    ...
    app.all('/weixin/pay', payMiddleware(initConfig).getNotify().done(function (message, req, res, next) {
    ...
    app.all('/', wechat(config, (req, res, next) => {
        var url = client.getAuthorizeURL("http://teach.wanviv.com/weixin/callback", '', 'snsapi_userinfo');
        console.log(url);
        res.reply({
            content: url,
            type: 'text'
 ...
 "privilege": [
   ...
   "PRIVILEGE2"
    ...
    app.get('/player/:_id', async(req, res, next) => {
        var player = await Player.model.findOne({
            _id: req.params._id
        }).exec();
        req.session.player = player;
    ...
    app.get('/signup/:_id', routes.views.signup.index);
    app.post('/signup/:_id', routes.views.signup.post);
